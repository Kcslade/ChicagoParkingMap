...
// ol set touch on map viewport
		var hasTouch = map.getViewport().classList.contains('ol-touch');
// Small screen
...
// popup all
		var popupall = new ol.control.Toggle({
			...
			if (active) {
				...
				map.on('pointermove', onPointerMove); // add qgis2web pointermove
...
//indicate button title
			...
			{						
				...
				//isSmall and hasTouch defined above
				if (isSmall && hasTouch) {
				search.collapse()
				}
			});
...
// popup all
			...
			/* var onSingleClick = function (evt) {
				...
				if (editBar.getActiveControls()[0]) {
					return;
				}
				if (measuring) {
					return;
				...
				// Verifica se la variabile spatialQuery è stata definita
				if (typeof spatialQuery !== 'undefined' && spatialQuery.getActive()) {
					return;
				}
...
//  ol-ext popup feature
		...
		// New highlight Features Iniziale
			...
			if (doHighlight && !doHover) {
				var highlightInteraction = new ol.interaction.Select({
					hitTolerance: 5,
					multi: true,
				...
				// Copio le features di highlightInteraction nel highlightLayer
		...
		var popup = new ol.Overlay.PopupFeature({
			...
			template: function (feature, layer) {
			  var layer = allLayers.find(function(layer) {
				if (layer instanceof ol.layer.Vector) {
				  return layer.getSource().getFeatures().indexOf(feature) > -1;
				}
			  });
			  ...
			  if (layer) {
				...
				/* 	// Acquisisci gli attributi che sono marcati come "hidden" in fieldImages o "hidden field" in fieldLabels
					// ed elimina l'attributo
					var hiddenAttributes = getHiddenAttributes(feature.getProperties(), fieldImages, fieldLabels);
					for (var i = 0; i < hiddenAttributes.length; i++) {
					  var key = hiddenAttributes[i];
...
/* // Function to get the list of hidden attributes for a feature
function getHiddenAttributes(properties, fieldImages, fieldLabels) {
  var hiddenAttributes = [];
  for (var key in properties) {
    // Check if the attribute is marked as hidden in fieldImages or hidden field in fieldLabels
    if ((fieldImages[key] && fieldImages[key].toLowerCase() === 'hidden') ||
        (fieldLabels[key] && fieldLabels[key].toLowerCase() === 'hidden field')) {
...
// ol-ext layerswitcher
		...
		// Legend Layer Attribution
			...
			// mostro il pulsante se diverso da schermo piccolo
			if (!isSmall) {
				$('.legend-attribution').css('display', 'block');
			}
					
			// mostro il pulsante selettivamente 
			$(document).ready(function() {
				$('.ol-layerswitcher').on('click', function() {
					if ($(this).hasClass('ol-forceopen')) {
						$('.legend-attribution').css('display', 'block');
					} else {
